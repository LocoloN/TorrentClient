...
template <typename Map> bool compareMaps(Map const &lhs, Map const &rhs) {
...
bencodeElem::bencodeElem(const bencodeElem &arg)
    : data(make_unique<bencodeDataType>(*arg.data)) {}
bencodeElem::bencodeElem(bencodeElem &&arg) noexcept
...
size_t bencodeElem::calculateStringSize() const {
  ...
  case bencodeKeySymbols::liststart: {
    auto &mylist = get<bencodeList>(*data);
    size_t resultSize(2);
  ...
  case bencodeKeySymbols::mapstart: {
    ...
    for (auto const &[key, val] : myMap) {
      ...
      if (current <= 0)
        throw runtime_error{string{"calculateStringSize error: key "} + key +
                            string{"length is <= 0"}};
      resultSize += current;

      current = val.calculateStringSize();
      if (current <= 0)
        throw runtime_error{string{"calculateStringSize error: val "} +
                            val.serialize() + string{"length is <= 0"}};
      resultSize += current;
    }
    return resultSize;
  }
...
std::string bencodeElem::serialize() const {
  ...
  case bencodeKeySymbols::liststart: {
    auto &blist = get<bencodeList>(*data);
    string result;
...
bencodeElem &bencodeElem::operator=(const bencodeElem &arg) {
  *data = *arg.data;
  return *this;
}
bencodeElem &bencodeElem::operator=(bencodeElem &&arg) noexcept {
  data = std::move(arg.data);
  return *this;
}
...
bool bencodeElem::operator==(const bencodeElem &param) const {
  if (getStoredTypeAsKey() != param.getStoredTypeAsKey())
    return false;
  ...
  case bencodeKeySymbols::mapstart:
    return compareMaps<bencodeDict>(*get_if<bencodeDict>(data.get()),
...
template <typename T> bencodeKeySymbols getKeyFromType();
template <> bencodeKeySymbols getKeyFromType<std::string>() {
  return bencodeKeySymbols::stringstart;
}
template <> bencodeKeySymbols getKeyFromType<int>() {
  return bencodeKeySymbols::intstart;
}
template <> bencodeKeySymbols getKeyFromType<std::vector<bencodeElem>>() {
